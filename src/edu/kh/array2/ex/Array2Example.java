...
public class Array2Example {
	...
	public void ex1() {
		...
		*/
		...
		System.out.println(Arrays.toString(arr)); // 주소값을 참조하고 있어서 주소값만 나옴

		// Arrays.deepToString(배열명)
		// -> 참고하고 있는 배열의 데이터가 나오는 부분까지 파고들어가서
		//    모든 값을 문자열로 반환
	...
	public void ex2() {
		...
		for(int row = 0; row < arr.length; row++) {
			int rowSum = 0; // sum이 밖에 있으면 전체 합이 나옴
			
			for(int col = 0; col < arr[row].length; col++) {
								// 0 0,1,2
								// 1 0,1,2
								// 2 0,1,2
				rowSum += arr[row][col];// 현재 행의 모든 열의 값을 누적	
			} System.out.printf("%d행의 합 : %d\n", row, rowSum);
		}
		System.out.println("=========================");
		
		// 열 별로 합 출력 -> 열부터 지정 후 각 행의 값을 누적
	...
	public void ex3() {
		...
		// 나중에 서로 크기가 다른 1차원 배열을 생성하여 창조하는 배열 
		
		char[][] arr = new char[4][];
						// char 2차원 배열 생성 시 행부분만 작성
